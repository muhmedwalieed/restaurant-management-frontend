  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-txt-muted mt-1">
            الطلبات النشطة للمطبخ، {activeBranch?.name || ''} (يتحدث تلقائيًا)
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 flex-wrap text-xs">
        {['ALL', 'CONFIRMED', 'PREPARING'].map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-3 py-2 rounded-full font-bold transition-colors ${
      ...
      {!activeBranchId ? (
        ...
        <EmptyState title="لا توجد طلبات نشطة" description="مطبخك نضيف، الطلبات المؤكدة/قيد التحضير هيظهر هنا." icon={ChefHat} />
      ) : (
              ...
              <div className="space-y-2">
                {order.items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-2 bg-bg-base border border-border-subtle rounded-lg px-3 py-2">
                    ...
                    {item.notes && <span className="text-xs text-txt-muted truncate max-w-[120px]">({item.notes})</span>}
                  </div>
                ))}
              </div>

              {order.notes && (
                <p className="text-xs text-txt-muted bg-bg-base border border-border-subtle rounded-md px-2 py-1 flex items-center gap-1">
