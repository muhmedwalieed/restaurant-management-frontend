  ...
  return (
    <div className="space-y-4">
      ...
      {(onSearchChange || filters || actions) && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-bg-surface p-4 border border-border-default rounded-lg">
          {/* Search Input */}
          {onSearchChange && (
      ...
      {isError && (
        <div className="bg-status-danger-bg border border-status-danger/30 rounded-lg p-6 text-center space-y-3">
          <div className="w-10 h-10 rounded-full bg-status-danger/10 text-status-danger flex items-center justify-center mx-auto">
      ...
      {!isError && (
        <div className="hidden md:block bg-bg-surface border border-border-default rounded-lg overflow-hidden shadow-sm">
                  ...
                  data.map((row, rIdx) => (
                    ...
                    >
                      {columns.map((col, cIdx) => (
                        <td key={col.key || cIdx} className="px-4 py-3.5 text-txt-primary whitespace-nowrap">
          ...
          {isLoading ? (
            ...
            data.map((row, idx) =>
              mobileCardRender ? (
                ...
                >
                  {columns.map((col, cIdx) => (
                    <div key={col.key || cIdx} className="flex items-center justify-between border-b border-border-subtle/50 pb-1.5 last:border-b-0 last:pb-0">
